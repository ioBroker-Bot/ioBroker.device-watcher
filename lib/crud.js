...
async function createData(adaptr, i) {
	try {
		...
		for (const [id] of Object.entries(devices)) {
			...
			tools.subscribeForeignStatesOnce(adaptr, instanceDeviceConnectionDP);
			tools.subscribeForeignObjectsOnce(adaptr, `${adaptr.selAdapter[i].selektor}`);
			...
			tools.subscribeForeignStatesOnce(adaptr, currDeviceString);
			...
			if (!deviceQualityDP.includes('undefined')) {
				...
				//subscribe to states
				tools.subscribeForeignStatesOnce(adaptr, deviceQualityDP);
			...
			if (deviceChargerState === undefined || deviceChargerState === false) {
				...
				if (!deviceBatteryStateDP.endsWith('undefined')) {
					tools.subscribeForeignStatesOnce(adaptr, deviceBatteryStateDP);
				}
				if (!isLowBatDP.endsWith('undefined') && isLowBatDP !== 'none') {
					tools.subscribeForeignStatesOnce(adaptr, isLowBatDP);
				}
				if (!faultReportingDP.endsWith('undefined')) {
					tools.subscribeForeignStatesOnce(adaptr, faultReportingDP);
				}
			...
			if (adaptr.selAdapter[i].stateValue != undefined) {
				...
				tools.subscribeForeignStatesOnce(adaptr, deviceStateSelectorHMRPC);
				tools.subscribeForeignStatesOnce(adaptr, rssiPeerSelectorHMRPC);
			}
			...
			// subscribe to states
			tools.subscribeForeignStatesOnce(adaptr, timeSelector);
			tools.subscribeForeignStatesOnce(adaptr, unreachDP);
				...
				if (!deviceUpdateDP.endsWith('undefined')) {
					if (!deviceUpdateDP.endsWith('none')) {
						if (!deviceUpdateDP.endsWith('null')) {
							tools.subscribeForeignStatesOnce(adaptr, deviceUpdateDP);
						}
					}
				}
			}
...
async function createAdapterUpdateData(adaptr, adapterUpdateListDP) {
	...
	tools.subscribeForeignStatesOnce(adaptr, adapterUpdateListDP);
